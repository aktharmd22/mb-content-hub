<x-app-layout>
    <x-slot name="header">Edit article type</x-slot>
    <x-slot name="title">Edit article type</x-slot>
    ...
    <div class="p-6 max-w-2xl">
        ...
        <div class="card">
            <div class="px-6 py-4 border-b border-ink-700">
                <h2 class="text-sm font-medium text-gray-100">Edit {{ $type->name }}</h2>
            </div>

            <form method="POST" action="{{ route('admin.article-types.update', $type) }}" class="px-6 py-5 space-y-4">
                @csrf
                @method('PATCH')
                @include('admin.article-types._form', ['type' => $type])
